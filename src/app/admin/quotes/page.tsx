...
export default function QuotesPage() {
  ...
  const load = async () => {
    try {
      // FIX: Added /admin prefix to match your folder structure
  ...
  const updateStatus = async (id: string, status: string) => {
    try {
      // FIX: Standardized to /admin/api/quotes
    ...
    <div>
            ...
            <tbody>
              {filtered.map(quote => (
                <tr key={quote.id} onClick={() => setSelected(quote)} style={{ borderBottom: '1px solid var(--border)', cursor: 'pointer' }}>
                  <td style={{ padding: '14px 20px', fontFamily: 'monospace', fontSize: '12px', color: 'var(--text-muted)' }}>#{quote.id.slice(-6)}</td>
                  <td style={{ padding: '14px 20px', fontWeight: '500' }}>{quote.user?.name ?? 'Guest'}</td>
                  <td style={{ padding: '14px 20px', color: 'var(--text-muted)' }}>{quote.user?.companyName ?? '—'}</td>
                  ...
                  <td style={{ padding: '14px 20px', fontWeight: '500' }}>${quote.total.toFixed(2)}</td>
      ...
      {selected && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }} onClick={() => setSelected(null)}>
          <div style={{ background: 'var(--surface)', borderRadius: '12px', width: '580px', maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto', border: '1px solid var(--border)' }} onClick={e => e.stopPropagation()}>
              ...
              <div>
                ...
                <div style={{ border: '1px solid var(--border)', borderRadius: '8px', overflow: 'hidden' }}>
                  {Array.isArray(selected.items) && selected.items.map((item: any, i: number) => (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', borderBottom: i < selected.items.length - 1 ? '1px solid var(--border)' : 'none' }}>
                      <div>
                      ...
                      <span style={{ fontWeight: '500' }}>${((item.price ?? 0) * (item.quantity ?? 1)).toFixed(2)}</span>
                    </div>
                  ))}
